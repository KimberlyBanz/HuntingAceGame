//*first, declare global keywords. let and const are both global and can't be
//be reassigned. [] is an array and {} are items within the array
...
//these are initialized as false but will change

let gameInProgress = false 
let shufflingInProgress = false 
let cardsRevealed = false

//This uses the DOM model
...
function endRound()
{
    ...
    },2000) //changed Timeout parameter
}
...
//this is an example of template literals, where you can import data from the html file as a parameter
...
//so that the code executes if you have tried to find one of the cards. My first use of .length function chaining IRL!!
...
// changed the output message slightly

function outputChoiceFeedBack(hit)
{
    if(hit)
    {
        updateStatusElement(currentGameStatusElem, "block", winColor, "That's a hit")
    }
    else
    {
        updateStatusElement(currentGameStatusElem, "block", loseColor, "Sorry you missed")
    }
}
...
function evaluateCardChoice(card)
{
  ...
  //aceId was set at the top of the programme
...
function loadGame(){
  ...
  //example of an arrow function expression, meant to simplify blocks of code
...
function checkForIncompleteGame()
{
    ...
    if(serializedGameObj)
    {
        ...
        {
            if(confirm('Would you like to continue with your last game?')) //this creates a popup dialog box in the browser
            {
                score = gameObj.score
...
function cardFlyInEffect()
{
    ...
    //gives a little bit of animation
...
{
  //Because the computer will normally create numbers with decimals, the "floor" will make a whole number
    ...
    // my first use of modulus! 
    //z index is what is on top
...
//push will add an object to an array and return the new length
